...
void UpdatePosition()
{
	...
	float z_freq=2.0f;
	float t_freq=3.0f;
	float x_freq_mult=1.18f;
	float y_freq_mult-2.12f;
	float t_freq_mult=3.13f;
	float amp=1.0f;
	float amp_mult=0.79f;
	float y=0.f;
	for int(i=0;i<6;i++)
	{
		y+= amp*sin( (inPosition.x*x_freq) + (inPosition.z*z_freq) + (time*t_freq) );
		amp=amp*amp_mult;
		x_freq=x_freq*x_freq_mult;
		z_freq=z_freq*z_freq_mult;
		t_freq=t_freq*t_freq_mult;
	}
	inPosition= vec3(inPosition.x,y,inPosition.z);
}
